...
const GLOBAL_CSS = `
  ...
  :root {
    --pai-parchment:  #F5F0E8;
    --pai-parchment2: #EDE7D9;
    --pai-ink:        #1C1C1A;
    --pai-ink-soft:   #3A3A36;
    --pai-forest:     #2D5016;
    --pai-forest-lt:  #3D6B1F;
    --pai-terra:      #B84A2E;
    --pai-gold:       #C8963C;
    --pai-muted:      #8A8578;
    --pai-border:     rgba(28,28,26,0.12);
    --pai-border2:    rgba(28,28,26,0.07);
    --pai-surface:    #FDFAF4;
  }

  .pai {
    font-family: 'Fira Code', monospace;
    background: var(--pai-parchment);
    color: var(--pai-ink);
    font-size: 15px;
    line-height: 1.85;
  }
  ...
  .pai-sec-rule { flex: 1; height: 1px; background: var(--pai-border); }
  .pai-h2 {
    font-family: 'Libre Baskerville', serif;
  ...
  .pai-p strong { font-weight: 500; color: var(--pai-ink); }
  .pai-p em { font-style: italic; color: var(--pai-ink); }
  .pai-p code {
    font-family: 'Fira Code', monospace;
    font-size: 13px;
  ...
  .pai-pull {
    margin: 48px 0;
    padding: 0 0 0 28px;
    border-left: 3px solid var(--pai-forest);
    position: relative;
  }
  .pai-pull-text {
    font-family: 'Libre Baskerville', serif;
  ...
  .pai-callout ul li {
    ...
    color: var(--pai-ink-soft); line-height: 1.65;
  }
  .pai-callout ul li::before {
    content: '→';
    color: var(--pai-forest); font-size: 12px;
  ...
  .pai-stat-label {
    ...
    color: var(--pai-muted); line-height: 1.5;
  }
  ...
  .pai-principle h3 {
    ...
    color: var(--pai-ink); margin-bottom: 8px; line-height: 1.3;
  }
  .pai-principle p {
    font-family: 'Fira Code', monospace;
    font-size: 12px; font-weight: 300;
    color: var(--pai-muted); line-height: 1.65; margin: 0;
  }
  ...
  .pai-table thead tr { background: var(--pai-forest); }
  .pai-table thead th {
    font-family: 'Fira Code', monospace;
  ...
  .pai-table tbody tr { border-bottom: 1px solid var(--pai-border2); }
  ...
  .pai-table td {
    ...
    font-family: 'Fira Code', monospace;
    font-weight: 300; color: var(--pai-ink-soft);
  }
  .pai-table td:first-child { font-weight: 500; color: var(--pai-ink); }
  ...
  .pai-case-body {
    padding: 24px 28px; background: var(--pai-surface);
  }
  ...
  .pai-ba {
    ...
    gap: 2px; background: var(--pai-border);
  ...
  .pai-ba-col.before .pai-ba-label { color: var(--pai-terra); }
  .pai-ba-col.after  .pai-ba-label { color: var(--pai-forest); }
  .pai-ba-text {
    font-family: 'Fira Code', monospace;
  ...
  .pai-ba-col.before .pai-ba-text { border-color: var(--pai-terra); }
  .pai-ba-col.after  .pai-ba-text { border-color: var(--pai-forest); }
  ...
  .pai-note {
    ...
    border-left: 2px solid var(--pai-gold);
  ...
  .pai-step {
    ...
    padding: 20px 0; border-bottom: 1px solid var(--pai-border2);
  }
  ...
  .pai-step-circle {
    ...
    background: var(--pai-forest); color: rgba(245,240,232,0.9);
    font-family: 'Libre Baskerville', serif;
  ...
  .pai-step-line {
    width: 1px; flex: 1; background: var(--pai-border); margin-top: 8px;
  }
  .pai-step-body h3 {
    font-family: 'Libre Baskerville', serif;
    font-size: 17px; font-weight: 700; color: var(--pai-ink);
    margin-bottom: 7px; margin-top: 4px; line-height: 1.3;
  }
  .pai-step-body p {
    font-family: 'Fira Code', monospace;
    font-size: 13px; font-weight: 300;
    color: var(--pai-muted); line-height: 1.75; margin: 0;
  }
  ...
  .pai-large-quote {
    ...
    line-height: 1.25; color: var(--pai-ink);
    padding: 40px 0;
...
const STATS = [
  ...
  {
    num: '7×',
    label: 'better outputs when users specify format, context, and audience explicitly',
  },
...
const PRINCIPLES = [
  {
    num: '01',
    title: 'Context Before Task',
    body: 'Always establish who, where, and why before asking what. Context is load-bearing structure, not decoration.',
  },
  {
    num: '02',
    title: 'Specificity Over Length',
    body: 'A 15-word precise prompt outperforms a 150-word vague one. Precision is a discipline, not a volume game.',
  },
  {
    num: '03',
    title: 'Format as Signal',
    body: 'Telling the AI how to respond — bullet list, paragraph, code, table — is as important as telling it what to respond.',
  },
  {
    num: '04',
    title: 'Persona & Audience',
    body: 'The best prompts name an audience. \'Explain this to a first-year medical student\' and \'explain this to a cardiologist\' are different instructions to the same question.',
  },
  {
    num: '05',
    title: 'Constraints Liberate',
    body: 'Adding constraints — word limits, excluded topics, required elements — actually improves output quality by narrowing the generative search space.',
  },
  {
    num: '06',
    title: 'Iteration is the Method',
    body: 'The first prompt is a hypothesis. The second is a revision. The third is usually good. Treating prompting as single-shot is the most common mistake.',
  },
];
...
function P({ children }) {
  return <p className="pai-p">{children}</p>;
}
...
export default function PromptAsInterface() {
  const title = 'Prompt as Interface';
  const description = 'Why writing is now a design skill — and how to teach users to communicate with AI.';

  return (
    <EditorialLayout
      toc={TOC}
      className="pai"
      progressColor="var(--pai-forest)"
      heroData={{
        eyebrow: 'Writing · AI Interfaces · Design Skill',
        title: 'Prompt as<br /><em>Interface</em>',
        subtitle:
          'Why writing is now a design skill — and how to teach users to communicate with AI.',
        meta: ['May 2026', '20 min read', 'Practitioner Essay'],
        tags: ['Prompt Design', 'UX Writing', 'AI Interfaces', 'Literacy', 'Onboarding'],
      }}
    >
      <SEO
        title={title}
        ...
        url="https://braxtonmandara.com/mandara/blogs/prompt-as-interface"
        type="article"
      />
      ...
      <Reveal className="editorial-reveal">
        <P>
          For most of computing history, the interface was a thing you could see. A button. A menu.
          A form. A layout. Designers made interfaces visible — legible, learnable, predictable.
          The medium was space: pixels arranged on a plane, affordances communicated through color
          and shape and position.
        </P>
        <P>
          Then language models arrived, and the interface became invisible. The primary mechanism
          for controlling a vast, probabilistic, generative system is now a text box — blank,
          patient, offering no affordances whatsoever. The entire interaction is mediated through
          words. And suddenly, writing is a design skill.
        </P>
        <P>
          Not writing in the traditional UX sense — not microcopy, not error messages, not
          onboarding text. Writing as the literal interface: the act of constructing a string of
          words that reliably produces the output you want from an AI system. This skill is now
          called <strong>prompt engineering</strong> by practitioners. It should be called
      ...
      <PullQuote attr="— The core thesis">
        "The prompt is not a search query. It is not a command. It is a design artifact — and like
        all design artifacts, its quality determines the quality of what it produces."
      </PullQuote>
      ...
      <SecMarker
        ...
        title="Writing Was Always Design"
      />
      <Reveal className="editorial-reveal">
        <P>
          The premise that writing is a design skill is not actually new. Every UX writer knows that
          words are interface elements — that the label on a button, the text in an error message,
          the copy in an onboarding flow are as much design decisions as the choice of typeface or
          the width of a sidebar. Words communicate affordances, set expectations, and guide
          behavior. They have always been design.
        </P>
        <P>
          What is new is that words are now <em>the entire interface</em> — not just the labels on
          the controls, but the controls themselves. In a conversational AI interface, there is no
          button to click, no menu to navigate, no form to fill. There is only the prompt. The
          quality of the experience is entirely determined by the quality of the writing.
        </P>
        <P>
          This is a radical inversion of the traditional UI paradigm. In conventional interfaces,
          designers carefully constrain and guide user input — radio buttons instead of free text,
          dropdowns instead of manual entry, confirmation dialogs instead of irreversible actions.
          The design absorbs complexity and offloads it from users. In prompt-driven interfaces,
          complexity is transferred back to the user. The blank text box says: <em>figure it out.</em>
        </P>
      </Reveal>
      ...
      <Reveal className="editorial-reveal">
        <P>
          This is not an indictment of AI product teams. It reflects the genuine difficulty of
          designing for open-ended generative systems — systems where the range of possible inputs
          and outputs is, for practical purposes, infinite. But it is a problem that design must
          own. If users cannot write prompts that produce useful outputs, they will not use the
          product. The failure is silent — not an error state, just a blank box and a sense of
          inadequacy — and it belongs to design.
        </P>
      </Reveal>
      ...
      <Reveal className="editorial-reveal">
        <P>
          Before designing for prompt literacy, it helps to be precise about what a prompt is and
          what it does. A prompt is not a command in the traditional computing sense — it is not a
          precise instruction with defined syntax that triggers a defined response. It is more like
          a context envelope: a packet of information that shapes the probability distribution over
          all possible AI responses.
        </P>
        <P>
          This probabilistic framing has important implications for how we think about prompt
          quality. A good prompt does not guarantee a good output — it shifts the probability
          distribution toward good outputs and away from bad ones. A bad prompt does not guarantee a
          bad output — it leaves the distribution wide, which means the output is unpredictable,
          likely generic, and often not what the user wanted.
        </P>
      </Reveal>
...
<span class="c-val">"...here's a previous summary that hit the right tone: [...]"</span>`}
      ...
      <Reveal className="editorial-reveal">
        <P>
          Most users supply only the task. They say what they want but not who they are, who they
          are writing for, what format they need, or what constraints apply. The AI fills in those
          blanks with its best statistical guess — which is to say, with the most common answer, not
          the most appropriate one. The result is generic. The user is disappointed. They blame the
          AI. The actual problem is a missing design pattern.
        </P>
      </Reveal>
      ...
      <BeforeAfter
        ...
        after={
          'Write a cover letter for a senior UX designer applying to a fintech startup (Series B, 40 people). The role is design systems lead. My background: 7 years in B2B SaaS, led 3 design systems from scratch. Tone: confident but not arrogant. Format: 3 short paragraphs, under 250 words. Avoid: buzzwords like \'passionate\' or \'innovative\'.'
        }
      />

      <Reveal className="editorial-reveal">
        <P>
          The difference between these two prompts is not effort — the longer one took about 90
          seconds to write. The difference is structural thinking: the second prompt has
          internalized the context envelope. It provides the AI with everything it needs to narrow
          its probability distribution toward a genuinely useful output.
        </P>
        <P>
      ...
      <Reveal className="editorial-reveal">
        ...
        <P>
          ...
          every major company, without exception — has consistently sold the experience as
          effortless. "Just ask." "Say what you need." "AI that understands you." These messages are
          not lies exactly, but they are optimistic about the current state of the technology in a
          way that sets users up for disappointment and self-blame when the blank box fails to
          produce magic.
        </P>
      </Reveal>

      <PullQuote attr="— On the failure of AI onboarding">
        "Every AI product tells users what the system can do. Almost none of them teach users how to
        actually ask for it."
      </PullQuote>
      ...
      <Reveal className="editorial-reveal">
        <P>
          ...
          interfaces spans a vast spectrum, and the choice of where to sit on that spectrum has
          profound implications for user capability, output quality, and product retention.
        </P>
      </Reveal>
      ...
      <Reveal className="editorial-reveal">
        <P>
          ...
          quality and user confidence within days, not months. The bottleneck is not human
          capability. It is the absence of deliberate teaching.
        </P>
        <P>
          ...
          iteration with immediate feedback), and through conceptual models (understanding why
          something works, not just that it works). Most AI onboarding fails on all three counts.
        </P>
      </Reveal>

      <ul className="pai-steps">
        {STEPS.map((s, i) => (
          <Reveal key={i} delay={i * 60} className="editorial-reveal">
      ...
      <Reveal className="editorial-reveal">
        <P>
          ...
          These are not tricks or hacks. They are applications of the underlying logic of how
          context envelopes work.
        </P>
      </Reveal>
...
a question instead of a statement."</span>`}
      ...
      <Reveal className="editorial-reveal">
        <P>
          ...
          of the next five years will not just expose AI capability — they will systematically
          transfer prompting skill to users through embedded, contextual, progressive pattern
          instruction.
        </P>
      </Reveal>
      ...
      <SecMarker
        ...
        title="Writing as Systems Thinking"
      />
      <Reveal className="editorial-reveal">
        <P>
        ...
        <P>
          This reframes prompt literacy as something much larger than a UX concern. It is a form of
          technological fluency — the equivalent, in the AI era, of knowing how to construct a
          search query, how to read a spreadsheet formula, or how to structure a database query.
          These skills were once rare, then became ubiquitous, then became invisible. Prompt
          literacy is on the same trajectory.
        </P>
      </Reveal>
      ...
      <CaseStudy title="Notion AI: Scaffolded Prompting Done Well">
        ...
        <P>
          ...
          served two purposes. Immediately, they gave users an obvious starting point. Over time,
          they taught users the vocabulary of AI interaction: that the AI responds to task framing,
          that "make shorter" is a legitimate instruction, that summarization is distinct from
          rewriting.
        </P>
        <P>
          The templates also had a secondary design function: they demonstrated the range of what
          was possible. Users who discovered "Explain this as a table" by clicking it once then
          started asking for tables spontaneously in the blank prompt box. The templates were not a
          replacement for prompt skill — they were a curriculum for developing it.
        </P>
      </CaseStudy>
      ...
      <SecMarker
        ...
        title="The Designer's New Deliverable"
      />
      <Reveal className="editorial-reveal">
        <P>
          If prompting is a design skill, then the designer's deliverable set needs to expand. The
          traditional outputs of a UX designer — wireframes, prototypes, component specs,
          interaction flows — remain necessary. But they are no longer sufficient for AI-powered
          products.
        </P>
      </Reveal>
      ...
      <Reveal className="editorial-reveal">
        <P>
          ...
          but as design specifications: what the prompt should contain, what output it should
          produce, and what variations are valid. It is a living artifact that bridges the design
          team, the engineering team, and eventually the user-facing pattern guidance that teaches
          prompt literacy in context.
        </P>
        <P>
          ...
          misunderstandings, the recommended recovery prompts, and the signals that indicate the
      ...
      <Reveal className="editorial-reveal">
        ...
        <P>
          ...
          now, millions of users are opening AI products every day, staring at blank text boxes,
          typing vague questions, receiving generic outputs, and quietly concluding that AI is not
          for them. This is a UX crisis masquerading as a technology limitation.
        </P>
        <P>
          The solution is not to wait for better AI. It is to design better onboarding, better
          scaffolding, better feedback loops, and better prompt literacy programs — to treat the gap
          between what users can ask and what AI can deliver as a design problem, not a user
      ...
      <PullQuote attr="— The core design opportunity">
        ...
        Every user who learns to prompt well is a design success. The difference between those
        outcomes is not the technology. It is the interface."
      </PullQuote>

      <Reveal className="editorial-reveal">
        <P>
          Writing is now a design skill. That means designers need to become better writers, yes —
          but more fundamentally, it means they need to become teachers of writing. The most
          impactful thing a UX team can do for an AI product in 2026 is not design a more beautiful
          empty state or a more elegant response card. It is to design the experience that turns a
          hesitant first-time user into a confident prompt author — someone who knows how to shape
          the probability distribution of a generative system toward what they actually need.
        </P>
        <P>
      ...
      <Reveal className="editorial-reveal">
        <div
          style={{
            marginTop: 80,
            paddingTop: 30,
            borderTop: '2px solid var(--pai-border)',
            display: 'flex',
        ...
        >
          ...
          >
            Prompt Design · UX Writing · May 2026
          </span>
          <button
